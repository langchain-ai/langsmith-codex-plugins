...
run_installer() {
  ...
  LAST_OUTPUT="$(
    PATH="$WORK/shim:$PATH" \
      LANGSMITH_CODEX_RELEASES_API="file://$WORK/$fixture.json" \
      LANGSMITH_CODEX_DOWNLOAD_BASE="file://$WORK/dl" \
...
expect_output "an unsupported platform gives the plugin commands" "The plugin does the same tracing and works on Windows and Linux.
