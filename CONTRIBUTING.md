...
Keep `shared-config.ts` byte-identical to the canonical Claude helper (SHA-256 `5e2d0dc374f2311db290ee74c9d3c65b44592496f3d9c4801a6b3d8d9be3b9c0`). Its `shared-config.test.ts` fixtures differ only in the relative import path. Report concrete helper bugs upstream rather than forking the contract or revalidating common fields with a harness schema. Shared fields are `enabled`, `defaultMuted`, `api_key`, `api_url`, `project`, `replicas`, `metadata`, `redact`, and `redact_extra_rules`; harness-only parent settings remain separate.
